...
- **Agent** — Flask REST API. Talks to Postfix via subprocess (`systemctl`, `mailq`, `postqueue`, `postsuper`, `tail`). Reads OAuth token files from disk. Streams logs via SSE (`tail -F`).
...
- **Config**: All `.env`, loaded by `python-dotenv`. No YAML/TOML/JSON config files.
